...
import useGlobalStore from '../../context/GlobalStateContext';
...
const JobCard = styled.div`
    ...
    ${props => props.decoration === "firstToApply" && "border: #FE889B 3px solid;"} ${props => props.decoration === "hurry" && "border: #FF6C4B 3px solid;"} ${props => props.decoration === "hurry" && "flex-direction: column-reverse;"} ${props => props.decoration === "social" && "flex-direction: column-reverse;"} @media(
    max-width: 500px) {
    flex-direction: column;
...
const Location = styled.div`
    ...
    text-overflow: ellipsis;
    max-height: 24px;
    ${props => props.hasCompany && `max-width: 120px;`} @media(
    min-width: 500px) {
...
const Job = (props) => {
    ...
    const {clickCountBySrc, setGlobalState} = useGlobalStore(state => ({
        clickCountBySrc: state.clickCountBySrc,
    ...
    const set_description = () => {
        ...
        try {
            if (job.job_metadata && typeof job.job_metadata._snippet == "string" && job.job_metadata._snippet.search(/cant_determine/igm) == -1) {
                // DescriptionStyle.WebkitLineClamp = "3";
                // description = job.job_metadata._snippet;
            }
        } catch (e) {
    ...
    try {
        if (typeof window !== 'undefined' && !window._jobViewMetricsState) {
            window._jobViewMetricsState = {
                sentJobImpressionIds: []
            };
        }
    } catch (e) {
    ...
    //
    // const trackJobViewEngagementMetrics = () => {
    //     try {
    //         const itemId = `${props.job.referencenumber}_${(props.job.searchMethod || "")}`;
    //         const sentJobImpressionIdsStr = localStorage.getItem("sentJobImpressionIds") || "";
    //         const sentJobImpressionIds = sentJobImpressionIdsStr.split(",");
    //
    //         if (!sentJobImpressionIds.includes(itemId)) {
    //             const metric = {
    //                 item: props.job,
    //                 name: "impression"
    //             };
    ...
    //
    //             const updatedImpressionIds = [...sentJobImpressionIds, itemId];
    //             if (updatedImpressionIds.length > 150) {
    //                 updatedImpressionIds.splice(0, updatedImpressionIds.length - 150);
    //             }
    //
    //             localStorage.setItem("sentJobImpressionIds", updatedImpressionIds.join(","));
    //         }
    //     } catch (e) {
    //         // Handle error
    //     }
    ...
    const trackJobViewEngagementMetrics = () => {
        ...
        const getStorageItem = (key) => {
            try {
                ...
                return window._jobViewMetricsState[key] || null;
            }
        };
        ...
        const setStorageItem = (key, value) => {
            try {
                ...
                // localStorage not supported, use localState
                window._jobViewMetricsState[key] = value;
            }
        };


        const arrayIncludes = (arr, item) => {
        ...
        try {
            var itemId = (props.job.referencenumber_with_location || props.job.referencenumber) +
                '_' + (props.job.searchMethod || '');
            if (!itemId) return;

            var sentJobImpressionIds = [];
            var sentJobImpressionIdsStr = getStorageItem("sentJobImpressionIds");

            if (typeof sentJobImpressionIdsStr === 'string') {
                sentJobImpressionIds = sentJobImpressionIdsStr.split(",");
            } else if (Array.isArray(sentJobImpressionIdsStr)) {
                sentJobImpressionIds = sentJobImpressionIdsStr;
            }

            // Remove empty entries
            sentJobImpressionIds = sentJobImpressionIds.filter(function (id) {
                return id !== null && id !== undefined && id !== '';
            });

            if (!arrayIncludes(sentJobImpressionIds, itemId)) {
                var metric = {
                    item: props.job,
                    name: "impression"
                };
                ...
                sentJobImpressionIds.push(itemId);

                if (sentJobImpressionIds.length > 150) {
                    sentJobImpressionIds = sentJobImpressionIds.slice(-150);
                }

                setStorageItem("sentJobImpressionIds", sentJobImpressionIds.join(","));
            }
        } catch (e) {
            console.error("Error in trackJobViewEngagementMetrics:", e);
        }
    };

    const trackJobView = () => {
        trackJobViewEngagementMetrics();
        ...
        try {
            ...
            if (jobViewHistory.search(props.referencenumber) > -1) {
                return;
            } else {
                jobViewHistory += "," + props.referencenumber;
        ...
        try {
            if (props.src && props.src.search(/api/igm) > -1) {
                return;
            }
        } catch (e) {
        ...
        try {
            ...
            try {
                params.jobBody = props.body.split(" ").splice(0, 50).join(" ");
            } catch (e) {
    ...
    const getTopDecorationTag = () => {
        let element;

        let decoration = props.decoration;

        if (props.src == "MyJobQuote") {
    ...
    const getInnerCard = () => {
        ...
        const {special_card, app, showEmailSignUpPopup, getProvidedEmail, getForcefullyHideEmailPrompts, job} = props;

        try {
            if (job.debug == true) {
                DescriptionStyle.WebkitLineClamp = 15;
                DescriptionStyle.maxHeight = "400px";
            }
        } catch (e) {
        ...
        const renderBenefits = () => {
            ...
            let _benefits = null;
            let show_benefits = false;

            try {
                if (ClientVars.AB && (ClientVars.AB.show_benefits == "on" || ClientVars.AB.show_benefits == "onV2")) {
                    show_benefits = true;
                }
            } catch (e) {
            ...
            try {
                if (job.job_metadata && job.job_metadata._benefits && Array.isArray(job.job_metadata._benefits) && job.job_metadata._benefits.length > 0) {
                    var regex = /cant_determine/igm;
                    _benefits = [];

                    for (var i = 0; i < job.job_metadata._benefits.length; i++) {
                        var benefit = job.job_metadata._benefits[i];
            ...
            if (_benefits && _benefits.length > 0 && show_benefits == true) {
                _benefits = _benefits.slice(0, 5);
                _benefits.push("View all benefits");
                return (
                    <div className={styles.job_benefits}>
                        {_benefits.map((benefit, index) => (
                            <div className={styles.job_benefit} key={index}>
        ...
        const baseCard = () => (
            <>
                <ContainerDiv>
                    <Title decoration={decoration} dangerouslySetInnerHTML={{__html: title}}></Title>
                    {(company || location) && (
                    ...
                    {logo && <img style={{maxWidth: logoMaxW, marginTop: "10px"}} src={logo} alt="Company Logo"/>}
                    {!hideDescription &&
                ...
                <Button decoration={decoration}>{CTA ? CTA : "View Salary & More Info"}</Button>
            </>
        );

        // Rendering a different card if special_card is defined and has a specific name
        if (special_card && special_card.name === "medium_card") {
            ...
            return (
                <MediumCard
                    ...
                    timeOut={true}
                    display={true}
                    uid={"uid"} // Adjust UID as necessary
                    ...
                    title={job.title}
                    buttonText={special_card.button_text || "View job benefits"}
                />
            );
        }

        // Default to base card if no special conditions are met
        return baseCard();
    };

    const get_close_btn = () => {

        const jobContainerCloseBtn = (e) => {
        ...
        let close_btn = <span className="jobContainerCloseBtnList" onClick={(e) => {
            e.stopPropagation();
            setOut(true);
    ...
    useEffect(() => {
        ...
        if (Logo) {
            setLogo(Logo.replace("http://", "https://"));
        }
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (tmpClickedJobsIdArr.indexOf(props.referencenumber) > -1) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (job_display_rules && Array.isArray(job_display_rules) == true && job_display_rules.length > 0) {

                for (let index in job_display_rules) {
                    let rule = job_display_rules[index];
                    if (rule == "tsn_max_clicks") {

                        //debugger
                        let maxClicks = 1;
                        let currentClicks = clickCountBySrc[src];
                        if (typeof currentClicks == "number" && isNaN(currentClicks) == false && currentClicks >= maxClicks) {
                            setDisplay(false);
                            logEvent("tsn_max_clicks", {maxClicks, currentClicks, src});
                        }
                    }
                }
            }
    ...
    const showSMSModal = () => {
        let sms_modal_display = false;
        let sms_modal_display_count = 0;
        ...
        try {
            ...
            let sms_modal_info = localStorage.getItem("sms_modal_info") || null;

            if (typeof sms_modal_info == "string" && sms_modal_info.length > 0) {
                return false;
            }
        ...
        try {
            if (ClientVars.AB.show_sms_modal_on_job_click != "on") {
                return false;
            }
        } catch (e) {
        ...
        try {
            sms_modal_display_count = localStorage.getItem("sms_modal_display_count") || 0;
            try {
                sms_modal_display_count = parseInt(sms_modal_display_count);
            } catch (e) {
        ...
        if (sms_modal_display_count < 1) {
            sms_modal_display = true;
        }

        try {
            if (sms_modal_display == true) {
                localStorage.setItem("sms_modal_display_count", sms_modal_display_count + 1);
            }
        } catch (e) {
        ...
        return sms_modal_display;
    }
    // Extracted logic for handling clicks
    const handleClick = (origin) => {


        try {

            if (typeof props.job == "object") {
                let metric = {
                    item: props.job,
                    name: "card_click"
                }
        ...
        try {
            window.currentJobClicked = props.job || this.props;
        } catch (e) {

        }

        try {
            props.incrementJobClickCount();
        } catch (e) {
        ...
        logEvent("click-job", {
            type: "click_button",
            origin: origin,
            redirectPath: job.url,
            decoration: props.decoration
        });


        let show_display_job_modal = display_job_modal();
        let showSMSModalFlag = false;

        try {
            //showSMSModalFlag = showSMSModal();
        } catch (e) {

        }
        //show_display_job_modal=true

        if (show_display_job_modal == true) {

            props.app.Job_Modal.current.setState({
                job: props.job || props,
            });

            props.app.Job_Modal.current.showModal(props);


        } else if (props.getProvidedEmail() || props.getForcefullyHideEmailPrompts()) {


            if (showSMSModalFlag == true) {
                let title = "Get Job Alerts On-the-Go!";
                let subtitle = "Sign up with your phone number and never miss out on a new opportunity.";
                props.app.showSmsModal("job_list", title, subtitle, job);
            } else {
                handleJobClick(job.url, job.title, job.src, job.cpc || job.ppc, null, job.body, "job_list", job);
            }


        } else {

            props.showEmailSignUpPopup(job.url, "job_list", job.title, job.src, job.body);

        }

        try {

            if (job.src.search(/serp/igm) == -1) {
                setIsJobClicked(true)
                ...
                tmpClickedJobsIdArr.unshift(props.referencenumber);

                tmpClickedJobsIdArr = tmpClickedJobsIdArr.slice(0, 20);
    ...
    return (
        <InView as="div"
            ...
            <div style={{
                ...
                bottom: "0px",
                right: "0px",
                display: isJobClicked ? "block" : "none",
                backgroundColor: isJobClicked ? "rgb(248 248 248 / 60%)" : "white"
            }}>
...
const arePropsEqual = (prevProps, nextProps) => {
    // Compare if 'title' prop has changed
    //console.log("arePropsEqual", prevProps, nextProps);
    if (prevProps.getProvidedEmail() !== nextProps.getProvidedEmail()) {
        return false;
    }

    if (prevProps.getForcefullyHideEmailPrompts() !== nextProps.getForcefullyHideEmailPrompts()) {
        return false;
    }


    // Perform additional prop comparisons as necessary...

    // If none of the conditions above triggered, props are equal,
    // so no need to re-render.
    return true;
};

const JobMemo = memo(Job, arePropsEqual);
