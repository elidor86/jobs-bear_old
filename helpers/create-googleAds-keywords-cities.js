const fs = require('fs');

let keywordCounter = 0;

let fileName = 'ca-locations.csv';
let fileNameOutput = 'ca-cities-keywords.txt';

const main = function () {

    const lineReader = require('readline').createInterface({
        input: fs.createReadStream(fileName)
    });

    lineReader.on('line', function (line) {


        if (line.length <= 1) {
            return;
        }

        let onlyNumReg = /^\d+$/;

        if (onlyNumReg.test(line) == true) {
            return;
        }

        let specialCharactersReg = /[^\da-zA-Z #$&_"+./:\-\[\]\']/g;

        if (specialCharactersReg.test(line) == true) {
            //console.log('specialCharactersReg ', line);
            //return;
        }

        if (line.search("'") > -1) {
            console.log('specialCharactersReg ', line);
            return;
        }

        line = line.replace(new RegExp("-", "igm"), " ");

        let lineJobs3 = "[" + line.split(",")[0].trim() + " jobs]"
        line = line.replace(new RegExp(",", "igm"), " ").replace(/\s{2,}/g, ' ');

        let lineJobs1 = "[Jobs in " + line + "]"
        let lineJobs2 = "[" + line + " jobs]"

        let lineJobs4 = "[Jobs in " + line.split(",")[0].trim() + "]"

        //console.log('lineJobs1 ', lineJobs1);
        //console.log('lineJobs2 ', lineJobs2);
        keywordCounter++;

        fs.appendFileSync(fileNameOutput, lineJobs1 + "\n");
        fs.appendFileSync(fileNameOutput, lineJobs2 + "\n");
        fs.appendFileSync(fileNameOutput, lineJobs3 + "\n");
        fs.appendFileSync(fileNameOutput, lineJobs4 + "\n");

    });

    lineReader.on('close', function (line) {

        console.log('keywordCounter :', keywordCounter);

    });


};


main();